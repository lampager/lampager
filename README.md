...
|:---|:---|:---|
| **`Paginator`** | Class | Fluent factory for building Query |
| **`AbstractProcessor`** | Abstract Class | Receive fetched records and format them |
| `ArrayProcessor` | Class | Simple Processor implementation for pure PDO |
| Query/`Query` | Class | SQL configuration container generated by Paginator |
| Query/... | Class | Children components of Query |
| Contracts/`Formatter` | Interface | Formatter interface pluggable to Processor |
| Concerns/`HasProcessor` | Trait | Helper for extended Paginator providing convenient accessibility to Processor |
...
- **`(array)`** __*$cursor*__<br> Associative array that contains `$column => $value`.<br>It must be **all-or-nothing**. Partial parameters are not allowed.
